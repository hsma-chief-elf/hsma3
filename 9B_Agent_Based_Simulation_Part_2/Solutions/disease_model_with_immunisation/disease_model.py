...
class Person_Agent(Agent):
    ...
    def __init__(self, unique_id, model, initial_infection, transmissibility, 
        ...
        # Store mean immunisation duration for random sampling when immunised
        ...
        if random.uniform(0, 1) < initial_infection:
            ...
            self.infected = False
        # Boolean flag to indicate whether the agent is currently immunised
        self.immunised = False
        # Attribute that stores the remaining duration of the agent's immunity
        self.immunisation_duration = 0
        # Attribute that stores the probability of being immunised in each
    ...
    # Agent immunisation method.  This method is called if the agent is
    # successfully immunised.  We update the Boolean immunised attribute to
    # True and randomly sample the duration for which they'll retain immunity,
    # using the mean duration that was passed in
